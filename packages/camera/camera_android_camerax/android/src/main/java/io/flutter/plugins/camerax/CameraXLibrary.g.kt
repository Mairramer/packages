...
abstract class CameraXLibraryPigeonProxyApiRegistrar(val binaryMessenger: BinaryMessenger) {
  ...
  /**
   * An implementation of [PigeonApiExtensionsManager] used to add a new Dart instance of
   * `ExtensionsManager` to the Dart `InstanceManager`.
   */
  abstract fun getPigeonApiExtensionsManager(): PigeonApiExtensionsManager
  ...
  fun setUp() {
    ...
    PigeonApiExtensionsManager.setUpMessageHandlers(
        binaryMessenger, getPigeonApiExtensionsManager())
  }
  ...
  fun tearDown() {
    ...
    PigeonApiExtensionsManager.setUpMessageHandlers(binaryMessenger, null)
  }
}
...
private class CameraXLibraryPigeonProxyApiBaseCodec(
  ...
  override fun writeValue(stream: ByteArrayOutputStream, value: Any?) {
    ...
    } else if (value is androidx.camera.extensions.ExtensionsManager) {
      registrar.getPigeonApiExtensionsManager().pigeon_newInstance(value) {
        if (it.isFailure) {
          logNewInstanceFailure("ExtensionsManager", value, it.exceptionOrNull())
        }
      }
    }
 ...
 */
enum class CameraXExtensionMode(val raw: Int) {
  /**
   ...
   *
   * See
   * https://developer.android.com/reference/androidx/camera/extensions/ExtensionMode#FACE_RETOUCH().
   */
  FACE_RETOUCH(5);

  companion object {
    fun ofRaw(raw: Int): CameraXExtensionMode? {
      return values().firstOrNull { it.raw == raw }
    }
  }
}
...
private open class CameraXLibraryPigeonCodec : StandardMessageCodec() {
  override fun readValueOfType(type: Byte, buffer: ByteBuffer): Any? {
    return when (type) {
      ...
      140.toByte() -> {
        return (readValue(buffer) as Long?)?.let { CameraXExtensionMode.ofRaw(it.toInt()) }
      }
      else -> super.readValueOfType(type, buffer)
    }
  }
 ...
 */
@Suppress("UNCHECKED_CAST")
abstract class PigeonApiExtensionsManager(
    open val pigeonRegistrar: CameraXLibraryPigeonProxyApiRegistrar
) {
  ...
  abstract fun getInstance(
      ...
      callback: (Result<androidx.camera.extensions.ExtensionsManager>) -> Unit
  )

  /** Returns whether the given extension mode is available for a camera. */
  abstract fun isExtensionAvailable(
      pigeon_instance: androidx.camera.extensions.ExtensionsManager,
      cameraSelector: androidx.camera.core.CameraSelector,
      mode: CameraXExtensionMode
  ): Boolean

  /** Returns a CameraSelector with the extension mode applied. */
  abstract fun getExtensionEnabledCameraSelector(
      pigeon_instance: androidx.camera.extensions.ExtensionsManager,
      cameraSelector: androidx.camera.core.CameraSelector,
      mode: CameraXExtensionMode
  ): androidx.camera.core.CameraSelector
  ...
  companion object {
    @Suppress("LocalVariableName")
    fun setUpMessageHandlers(binaryMessenger: BinaryMessenger, api: PigeonApiExtensionsManager?) {
      val codec = api?.pigeonRegistrar?.codec ?: CameraXLibraryPigeonCodec()
      run {
        val channel =
            BasicMessageChannel<Any?>(
                binaryMessenger,
                "dev.flutter.pigeon.camera_android_camerax.ExtensionsManager.getInstance",
                codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val providerArg = args[0] as androidx.camera.lifecycle.ProcessCameraProvider
            api.getInstance(providerArg) {
                result: Result<androidx.camera.extensions.ExtensionsManager> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(CameraXLibraryPigeonUtils.wrapError(error))
              } else {
      ...
      run {
        val channel =
            BasicMessageChannel<Any?>(
                binaryMessenger,
                "dev.flutter.pigeon.camera_android_camerax.ExtensionsManager.isExtensionAvailable",
                codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val pigeon_instanceArg = args[0] as androidx.camera.extensions.ExtensionsManager
            val cameraSelectorArg = args[1] as androidx.camera.core.CameraSelector
            val modeArg = args[2] as CameraXExtensionMode
            val wrapped: List<Any?> =
                try {
                  listOf(api.isExtensionAvailable(pigeon_instanceArg, cameraSelectorArg, modeArg))
                } catch (exception: Throwable) {
                  CameraXLibraryPigeonUtils.wrapError(exception)
                }
            reply.reply(wrapped)
      ...
      run {
        val channel =
            BasicMessageChannel<Any?>(
                binaryMessenger,
                "dev.flutter.pigeon.camera_android_camerax.ExtensionsManager.getExtensionEnabledCameraSelector",
                codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
                ...
                try {
                  listOf(
                      api.getExtensionEnabledCameraSelector(
                          pigeon_instanceArg, cameraSelectorArg, modeArg))
                } catch (exception: Throwable) {
                  CameraXLibraryPigeonUtils.wrapError(exception)
                }
            reply.reply(wrapped)
  ...
  @Suppress("LocalVariableName", "FunctionName")
  /** Creates a Dart instance of ExtensionsManager and attaches it to [pigeon_instanceArg]. */
  fun pigeon_newInstance(
      pigeon_instanceArg: androidx.camera.extensions.ExtensionsManager,
      callback: (Result<Unit>) -> Unit
  ) {
    if (pigeonRegistrar.ignoreCallsToDart) {
      callback(
      ...
      val codec = pigeonRegistrar.codec
      val channelName =
          "dev.flutter.pigeon.camera_android_camerax.ExtensionsManager.pigeon_newInstance"
      val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
      channel.send(listOf(pigeon_identifierArg)) {
        if (it is List<*>) {
          if (it.size > 1) {
